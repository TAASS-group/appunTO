...
const Page = () => {
  ...
  const fetchQuestion = async (question_id: string) => {
    const res = await fetch(`http://localhost:8080/question/getQuestionById/${question_id}`);
    if (!res.ok) {
  ...
  const fetchAnswers = async (question_id: string) => {
    const res = await fetch(`http://localhost:8080/answer/getAll/${question_id}`);
    if (!res.ok) {
      throw new Error('Network response was not ok');
    }
    const data = await res.json();
    return data.map(({ answer, username, imageUrl }: { answer: AnswerType, username: string, imageUrl: string }) => ({
      text: answer.text,
      id: answer.id,
  ...
  const { data: question , error: questionError, isLoading: questionLoading } = useQuery(['question', question_id], () => fetchQuestion(question_id));
  const { data: answers, error: answersError, isLoading: answersLoading } = useQuery(['answers', question_id], () => fetchAnswers(question_id));
